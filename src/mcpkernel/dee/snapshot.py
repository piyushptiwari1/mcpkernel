...
from mcpkernel.utils import hash_directory, sha256_hex, sha256_json
...
def take_environment_snapshot(
    *,
    ...
    if include_env_vars:
        # Only include MCPKERNEL_* and a curated set to avoid noise
        filtered_env = {
            k: v for k, v in sorted(os.environ.items()) if k.startswith("MCPKERNEL_") or k in _TRACKED_ENV_VARS
        }
